...
function makePDF() {
    ...
    <div style="width:210mm; padding:12mm; background:#fff; font-family:sans-serif;">
        <div style="border:2px solid #000; padding:25px; min-height:265mm; display:flex; flex-direction:column;">
            ...
            <div style="flex:1;">
                <div style="font-size:18px; font-weight:900;">REVOLVER OBEN</div>${headerRow}${o.map(getRow).join('')}
                ${u.length ? `<div style="margin-top:35px; font-size:18px; font-weight:900;">REVOLVER UNTEN</div>${headerRow}${u.map(getRow).join('')}` : ''}
            </div>
        </div>
    </div>`;
