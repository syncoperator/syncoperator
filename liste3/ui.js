    ...
    tools.forEach((t, i) => {
        ...
        item.innerHTML = `
            ...
            <div style="flex:1; min-width:0;" onclick="modalT(${i})">
                ...
                <b style="font-size:18px; line-height:1.2;">${t.nm || '---'}</b>
            </div>
        `;
...
// --- IMPORT / EXPORT ---
function runImp() {
    const area = el('imp-area');
    const text = area.value.trim();
    if(!text) return;
    
    // Простой парсинг списка инструментов (T-Nr \t Beschreibung)
    const lines = text.split('\n');
    const newTools = lines.map(line => {
        const parts = line.split('\t');
        return { id: (parts[0] || '').trim(), nm: (parts[1] || '').trim(), dia: '', rev: false };
    }).filter(t => t.id || t.nm);
...
function exportJSON() { el('imp-area').value = JSON.stringify(db); el('imp-area').select(); document.execCommand('copy'); alert("JSON in Zwischenablage kopiert!"); }
function importJSON() { try { const parsed = JSON.parse(el('imp-area').value); if(Array.isArray(parsed)) { db = parsed; localStorage.setItem(DB_KEY, JSON.stringify(db)); renderList(); hide('m-imp'); } } catch(e) { alert("Fehler beim JSON-Import"); } }
function deleteProject(i) { if(confirm('Dieses Projekt unwiderruflich löschen?')) { db.splice(i, 1); localStorage.setItem(DB_KEY, JSON.stringify(db)); renderList(); } }
