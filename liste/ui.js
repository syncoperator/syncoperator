...
function renderTools() {
    ...
    tools.forEach((t, i) => {
        ...
        // Проверка на многострочность толеранса
        const isMultiLine = (t.dia || '').includes('\n');
        const alignStyle = isMultiLine ? 'center' : 'flex-start';

        item.innerHTML = `
            <div style="flex:1; padding-right:15px; display:flex; flex-direction:column; justify-content:center; align-self:${alignStyle}; min-width:0;" onclick="modalT(${i})">
            ...
            <div style="font-weight:900; color:var(--accent); font-size:18px; text-align:right; white-space:pre-line; align-self:${alignStyle}; min-width:110px;">${t.dia}</div>
        `;
        ...
        item.ondrop = (e) => {
            e.preventDefault();
            moveTool(parseInt(e.dataTransfer.getData('text/plain')), i);
        };
...
// --- PDF С УМНОЙ ЦЕНТРОВКОЙ ---
function makePDF() {
    const p = db[currentIdx];
    const rows = (p.tools || []).map(t => {
        const isMulti = (t.dia || '').includes('\n');
        const valign = isMulti ? 'center' : 'flex-start';
        const ptop = isMulti ? '0' : '4px'; // Небольшой отступ сверху, если одна строка

        return `
        <div style="display:flex; align-items:${valign}; border-bottom:1px solid #eee; padding:8px 0; min-height:40px;">
            <div style="width:75px; font-weight:800; font-size:14px; font-family:sans-serif; padding-top:${ptop};">${t.id}</div>
            <div style="flex:1; font-weight:700; font-size:14px; text-transform:uppercase; font-family:sans-serif; padding-right:10px; white-space:pre-wrap; line-height:1.2; padding-top:${ptop};">${t.nm}</div>
