...
// ИСПРАВЛЕННАЯ КНОПКА +NEU
function modalP(edit = false) {
    // Если создание нового — обнуляем все, если правка — берем данные из базы
...
function runImp() {
    ...
    text.split('\n').forEach(line => {
        if(line.trim()) db[currentIdx].tools.push({ id:'T?', nm: line.trim().toUpperCase(), dia:'' });
    });
    localStorage.setItem(DB_KEY, JSON.stringify(db));
    el('imp-area').value = '';
    renderTools();
...
// ОБНОВЛЕННЫЙ СТИЛЬНЫЙ PDF
function makePDF() {
    const p = db[currentIdx];
    const rows = p.tools.map(t => `
        <div style="display: flex; align-items: baseline; border-bottom: 1px solid #f0f0f0; padding: 12px 0;">
            <div style="width: 70px; font-weight: 800; font-size: 15px; color: #000;">${t.id}</div>
            <div style="flex: 1; font-weight: 700; font-size: 15px; text-transform: uppercase; color: #000;">${t.nm}</div>
            <div style="width: 90px; text-align: right; font-weight: 800; font-size: 15px; color: #000;">${t.dia}</div>
        </div>
    `).join('');
        ...
        <div style="border: 2px solid #000; padding: 25px; min-height: 265mm; display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 5px;">
                <div>
                    ...
                    <div style="font-size: 68px; font-weight: 900; line-height: 0.85; letter-spacing: -2px;">${p.num}</div>
                </div>
                <div style="width: 190px; font-size: 11px; font-weight: 800; line-height: 1.6;">
            ...
            <div style="display: flex; font-size: 10px; font-weight: 900; text-transform: uppercase; margin-bottom: 5px;">
                ...
                <div style="width: 90px; text-align: right;">Ø / TOLERANZ</div>
            </div>
            <div style="border-bottom: 1.5px solid #f0f0f0; margin-bottom: 0px;"></div>
