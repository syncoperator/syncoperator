...
/* FORCED MODES */
.force-mobile .dsk-sim,.force-mobile .dsk-ke,.force-mobile .dsk-tools,.force-mobile .dsk-editor{display:flex !important;flex-direction:column !important;height:100%;padding:0 !important;}
.force-mobile .dsk-sim-main,.force-mobile .dsk-ke-main{flex:1;display:flex;flex-direction:column;min-height:0;}
.force-mobile .dsk-sim-side,.force-mobile .dsk-ke-side,.force-mobile .dsk-tools-list{display:none !important;}
.force-desktop .dsk-sim{display:grid !important;grid-template-columns:1fr 380px !important;gap:12px;}
.force-desktop .dsk-sim-side{display:flex !important;flex-direction:column;gap:12px;overflow-y:auto;padding:0 6px;}
.force-desktop .side-panel{background:var(--p);border-radius:14px;padding:14px;box-shadow:var(--shadow-sm);border:1px solid var(--b1);}
.force-desktop .side-title{font-size:12px;font-weight:700;color:var(--inkM);letter-spacing:.5px;text-transform:uppercase;margin-bottom:10px;display:flex;align-items:center;gap:6px;}
.force-desktop .side-title::before{content:"";width:4px;height:14px;background:var(--blue);border-radius:2px;}
`;
...
const[viewMode,setViewMode]=useState(typeof window!=="undefined"&&window.innerWidth>=1024?"desktop":"mobile");const[sp,setSp]=useState("RO");const[cRO,setCRO]=useState(PRO);const[cRU,setCRU]=useState(PRU);const[tab,setTab]=useState("sim");const[toolsRO,setToolsRO]=useState({});const[toolsRU,setToolsRU]=useState({});const[customRO,setCustomRO]=useState([]);const[customRU,setCustomRU]=useState([]);const[dia,setDia]=useState(55);const[sel,setSel]=useState("T01");const[step,setStep]=useState(-1);const[run,setRun]=useState(false);const[showG0,setShowG0]=useState(true);const[visRO,setVisRO]=useState(()=>new Set());const[visRU,setVisRU]=useState(()=>new Set());const sRef=useRef(null);
...
const blocks=parse(code),baseTools=getTools(blocks),allPaths=getPaths(blocks);const customs=sp==="RO"?customRO:customRU;const toolList=[...baseTools,...customs.filter(c=>!baseTools.find(t=>t.key===c.key))];
...
return(<><style>{G}</style><div className={"app force-"+viewMode}>
<div className="hdr">
<div className="hdr1">
...
<button onClick={()=>setViewMode(v=>v==="desktop"?"mobile":"desktop")} style={{padding:"6px 12px",margin:"8px 8px 8px 0",border:"1px solid var(--b1)",borderRadius:8,background:"var(--r)",color:"var(--inkS)",fontFamily:"JetBrains Mono",fontSize:11,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",letterSpacing:.3}}>{viewMode==="desktop"?"🖥 DT":"📱 MB"}</button><div className="statrow">{[{l:"MOVES",v:allPaths.length,c:"var(--blue)"},{l:"TOOLS",v:toolList.length,c:"var(--amber)"},{l:"BLOCKS",v:blocks.filter(b=>b.type==="block").length,c:"var(--cyan)"}].map(({l,v,c})=>(<div key={l} className="stat"><div className="statv" style={{color:c}}>{v}</div><div className="statl">{l}</div></div>))}<div className="stat"><input type="number" step={0.5} value={dia} onChange={e=>setDia(parseFloat(e.target.value)||0)} className="bi" onFocus={e=>e.target.style.borderColor="var(--blue)"} onBlur={e=>e.target.style.borderColor="var(--b1)"}/>  <div className="statl">Ø MM</div></div></div>
</div>
...
<div className="main">
{tab==="sim"&&(<div className="dsk-sim" style={{minHeight:0}}><div className="dsk-sim-main">
...
<div style={{width:185,borderLeft:"1px solid var(--b1)",overflowY:"auto",padding:8,background:"var(--g)"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}><div style={{fontSize:10,color:"var(--inkM)",fontFamily:"Inter",letterSpacing:.5,textTransform:"uppercase",fontWeight:700}}>TOOLS · {sp}</div><button onClick={()=>{const nums=toolList.map(t=>t.num);let n=1;while(nums.includes(n))n++;const key="T"+String(n).padStart(2,"0");const newTool={key,num:n,comment:"Custom "+key};const setC=sp==="RO"?setCustomRO:setCustomRU;setC(p=>[...p,newTool]);setToolDb(prev=>({...prev,[key]:{...DT,name:key+" Custom"}}));setSel(key);}} style={{width:24,height:24,padding:0,borderRadius:6,border:"1px solid rgba(0,200,150,.3)",background:"var(--greenL)",color:"var(--green)",fontSize:16,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button></div><div style={{display:"flex",flexDirection:"column",gap:4}}>{toolList.map((t,i)=>{const col=cols[t.key]||TC[i%TC.length],td=toolDb[t.key],active=sel===t.key;return(<div key={t.key} onClick={()=>setSel(t.key)} style={{border:"1px solid "+(active?col:"var(--b1)"),borderRadius:8,padding:"8px 9px",cursor:"pointer",background:active?col+"15":"var(--r)",transition:"all .1s"}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:22,height:22,borderRadius:5,background:col,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{color:"#fff",fontSize:8,fontWeight:800,fontFamily:"Space Mono"}}>{t.key}</span></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:11,fontWeight:700,color:active?col:"var(--ink)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{td?.name||t.comment||t.key}</div><div style={{fontSize:9,color:"var(--inkM)",fontFamily:"Space Mono",marginTop:1}}>{TL[td?.type]||"—"}</div></div></div></div>);})}</div></div>
</div>)}
